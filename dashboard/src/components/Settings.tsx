...
export function Settings({ Roomnumber, ...initalProps }: SettingsProps): React.ReactElement {
	const [roomName, setRoomName] = useState(initalProps.roomName);
	const [sot, setSot] = useState(initalProps.sot);
	const [sct, setSct] = useState(initalProps.sct);
	const [beamerIP, setBeamerIP] = useState(initalProps.beamerIP);
	...
	const onSave = () => {
		...
		console.log({ sot, sct, beamerIP, beamerCOM, roomName: roomName || '' });
	};
	return (
